...
export default function ProductSummaryEquipment({
  productData,
  onSubmit,
  onPrevious,
  isLoading,
  ...
  const formData = {
    ...
    warrantyInfo: productData?.batch?.warrantyInfo || "",
    serviceSchedule: productData?.batch?.serviceSchedule || "",
    certification: productData?.batch?.certification || null,
    technicalSpecifications: productData?.batch?.technicalSpecifications || "",
    userManuals: productData?.batch?.userManuals || null,
  ...
  const calculateInventoryValue = () => {
    return (parseFloat(formData.costPrice) * parseFloat(formData.quantity)).toFixed(2);
  };
    ...
    <div className={`px-6 ${isPrintMode ? 'print-mode' : ''}`}>
      ...
      <div className="bg-blue-50 border-l-4 border-primary/50 p-4 mb-6">
        <div className="flex items-start">
          ...
          <div className="ml-3">
            <h3 className="text-sm font-medium text-primary">Equipment Summary - Review Before Submission</h3>
            <div className="mt-2 text-sm text-primary">
              <p>Please carefully review all equipment information before finalizing your submission.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-1">Spare Part Information</label>
              {formData.sparePartInfo.length > 0 ? (
                <div className="text-base border border-gray-300 rounded-md p-2 bg-gray-50">
                  <ul className="list-disc pl-5">
                    {formData.sparePartInfo.map((part, index) => (
                      <li key={index}>{part}</li>
                    ))}
                  </ul>
                </div>
              ) : (
                <div className="text-base border border-gray-300 rounded-md p-2 bg-gray-50 text-gray-500 italic">
                  No spare part information available
                </div>
              )}
            </div>
          </div>
      ...
      <div className="mb-8">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-6">
          <div className="bg-white p-4 border rounded-lg shadow-sm">
            ...
            <p className="text-2xl font-bold text-gray-800">{formData.quantity}</p>
            <p className="text-xs text-gray-500 mt-1">Total units in inventory</p>
          </div>
          
          <div className="bg-white p-4 border rounded-lg shadow-sm">
            <h4 className="text-sm font-medium text-gray-500 mb-1">Cost Price</h4>
            <p className="text-2xl font-bold text-gray-800">${parseFloat(formData.costPrice).toFixed(2)}</p>
            <p className="text-xs text-gray-500 mt-1">Per unit</p>
          </div>
          
          <div className="bg-white p-4 border rounded-lg shadow-sm">
            <h4 className="text-sm font-medium text-gray-500 mb-1">Selling Price</h4>
            <p className="text-2xl font-bold text-gray-800">${parseFloat(formData.sellingPrice).toFixed(2)}</p>
            <p className="text-xs text-gray-500 mt-1">Per unit</p>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-6">
          <div className="bg-white p-4 border rounded-lg shadow-sm">
            ...
            <p className="text-2xl font-bold text-gray-800">
              <span className={parseFloat(profitMargin) > 20 ? "text-green-600" : "text-amber-600"}>
                {profitMargin}%
              </span>
            </p>
            <p className="text-xs text-gray-500 mt-1">Per unit</p>
          ...
          <div className="bg-white p-4 border rounded-lg shadow-sm">
            <h4 className="text-sm font-medium text-gray-500 mb-1">Total Inventory Value</h4>
            <p className="text-2xl font-bold text-gray-800">${calculateInventoryValue()}</p>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div>
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-1">Service Schedule/Maintenance</label>
              <div className="text-base border border-gray-300 rounded-md p-2 bg-gray-50 min-h-20">
      ...
      <div className="mb-8">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          {formData.userManuals && (
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-1">User Manuals</label>
              <div className="border border-gray-300 rounded-md p-2 bg-gray-50">
      ...
      <div className="hidden print:block mt-8 pt-4 border-t border-gray-300 text-sm text-gray-500">
        ...
        <p>This is a summary of equipment details prior to inventory submission.</p>
      </div>
