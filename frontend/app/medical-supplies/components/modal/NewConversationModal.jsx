...
import { SEARCH_MS_USERS } from "../../api/graphql/queries";
...
export const NewConversationModal = ({
  ...
  const users = data?.searchMSUsers || [];
  ...
  const handleUserSelect = async (selectedUser) => {
    ...
    try {
      setCreating(true);
      const newConversation = await createConversation([selectedUser.userId]);
            ...
            <div className="divide-y divide-gray-100">
              {users.map((user) => (
                <div
                  ...
                  onClick={() => !creating && handleUserSelect(user)}
                >
                  <div className="flex items-center">
                    ...
                    <div className="w-12 h-12 rounded-full overflow-hidden flex-shrink-0">
                      {user.profileImageUrl ? (
                        <img
                          src={user.profileImageUrl.startsWith("http") ? user.profileImageUrl : process.env.NEXT_PUBLIC_MEDICAL_SUPPLIES_API_URL+user.profileImageUrl}
