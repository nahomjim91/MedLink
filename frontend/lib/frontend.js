  ...
  const api = {
    ...
    uploadFile: async (file, appointmentId) => {
      try {
        ...
        const response = await fetch(`${telehealthBackendUrl}/api/chat/upload`, {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
          },
    ...
    getChatStats: async () => {
      try {
        const response = await fetch(`${telehealthBackendUrl}/api/chat/stats`, {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
          },
...
import { Search, MessageCircle, Send, Check, Video, Clock, Calendar, User, ArrowLeft, Menu, Upload, FileText } from 'lucide-react';
...
const MedicalChatInterface = ({ appointmentId }) => {
  ...
  const handleSendMessage = async () => {
    if (!message.trim() || !activeAppointment || !chatAccess?.canSendMessages) return;
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      // Upload file
      const uploadResult = await api.uploadFile(file, activeAppointment.appointmentId);
  ...
  const canSendMessage = (status) => {
    return status === AppointmentStatus.IN_PROGRESS && chatAccess?.canSendMessages;
  };
  ...
  const currentMessages = activeAppointment ? (messages[activeAppointment.appointmentId] || []) : [];
  
  // Get typing users for current appointment
  const typingUsers = activeAppointment ? getTypingUsers(activeAppointment.appointmentId) : [];
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      {!isConnected && (
        <div className="absolute top-0 left-0 right-0 bg-red-500 text-white text-center py-2 z-50">
          <span className="text-sm">Disconnected from chat server. Attempting to reconnect...</span>
        </div>
      )}
            ...
            filteredChats.map((chat) => (
              <div
                ...
                className={`flex items-center p-4 cursor-pointer hover:bg-gray-50 ${
                  activeChat?.appointmentId === chat.appointmentId ? "bg-teal-50 border-r-2 border-teal-500" : ""
                }`}
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex items-center space-x-1">
                      ...
                      <span className="text-xs text-gray-500">
                        {chat.lastMessageTime || new Date(chat.updatedAt || Date.now()).toLocaleTimeString("en-US", {
                          hour: "numeric",
                          minute: "2-digit",
                          hour12: true,
                        })}
            ...
            <div className="bg-white border-b border-gray-200 p-4">
              <div className="flex justify-between items-center">
                <div className="flex items-center">
                  ...
                  <div className="relative">
                    <div className="w-10 h-10 bg-teal-100 rounded-full flex items-center justify-center">
                      {activeChat.avatar ? (
                        ...
                        <span className="text-teal-600 font-semibold text-sm">
                          {getInitials(activeChat.doctorName || activeChat.patientName)}
                        </span>
                      )}
                    </div>
                    {activeChat.isOnline && (
                  ...
                  <div className="ml-3">
                    <h2 className="text-lg font-semibold text-gray-900">
                      {activeChat.doctorName || activeChat.patientName || 'Unknown'}
                    </h2>
                    <p className="text-sm text-gray-500">
                      {isConnected ? (activeChat.isOnline ? "Online" : "Offline") : "Disconnected"}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  {activeAppointment?.status === AppointmentStatus.IN_PROGRESS && (
                    <button
                      onClick={() => requestExtension(activeAppointment.appointmentId)}
                ...
                <div className="mt-4">
                  <div className="bg-gray-50 rounded-lg p-3">
                    <div className="flex justify-between items-start mb-2">
                      <span className="text-xs text-gray-500">{activeAppointment.appointmentNumber}</span>
                      <span className={`text-xs px-2 py-1 rounded-full ${getStatusColor(activeAppointment.status)}`}>
                ...
                <>
                  ...
                  {currentMessages.map((msg) => (
                    <div
                      key={msg.messageId}
                      className={`mb-4 flex ${
                        msg.senderId === currentRoom?.userId ? "justify-end" : "justify-start"
                      }`}
                    >
                      <div
                      ...
                      >
                        {msg.fileUrl ? (
                          <div className="space-y-2">
                            ...
                            {msg.textContent && (
                              <p className="text-sm break-words">{msg.textContent}</p>
                            )}
                          </div>
                        ) : (
                          <p className="text-sm break-words">{msg.textContent}</p>
                        ...
                        <div className={`flex items-center mt-1 ${
                          ...
                          <span className={`text-xs ${
                            msg.senderId === currentRoom?.userId ? "text-teal-100" : "text-gray-500"
                          }`}>
              ...
              <div className="bg-white border-t border-gray-200 p-4">
                <div className="flex items-center space-x-3">
                  ...
                  <button
                    ...
                    disabled={!message.trim() || !isConnected || !chatAccess?.canSendMessages}
            ...
            {activeAppointment && !canSendMessage(activeAppointment.status) && (
              <div className="bg-gray-100 border-t border-gray-200 p-4 text-center">
                <p className="text-sm text-gray-600">
                  {getStatusMessage(activeAppointment.status) || "Messaging is not available for this appointment"}
                </p>
              </div>
            )}
          </>
          ...
          <div className="flex-1 flex items-center justify-center bg-gray-50">
            <div className="text-center px-4">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">
                {isConnected ? "Select a conversation" : "Connecting to chat..."}
              </h3>
