...
const CartModel = {
    ...
    try {
      ...
      const cartItemsWithBatches = await Promise.all(
        cartItems.map(async (item) => {
          const batchItems = await this.getCartBatchItemsByCartItemId(item.cartItemId);
  ...
  async addToCart(userId, input) {
    try {
      ...
      if (totalAvailable < quantity) {
        throw new Error(`Not enough quantity available. Requested: ${quantity}, Available: ${totalAvailable}`);
      }
      ...
      return await this.addBatchAllocationsToCart(
        ...
        product.name,
        product.productType,
        product.imageList && product.imageList.length > 0 ? product.imageList[0] : null,
        batchAllocations
      );
    } catch (error) {
  ...
  async addSpecificBatchToCart(userId, input) {
    try {
      ...
      if (batch.quantity < quantity) {
        throw new Error(`Not enough quantity available. Requested: ${quantity}, Available: ${batch.quantity}`);
      }
   ...
   * @returns {Object} Updated cart
   */
  async addBatchAllocationsToCart(userId, productId, productName, productType, productImage, batchAllocations) {
