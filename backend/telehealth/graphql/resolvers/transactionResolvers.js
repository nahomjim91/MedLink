...
const { TransactionModel, RefundModel } = require("../../models/transactionAndRefund");
...
const transactionResolvers = {
  ...
  Query: {
    ...
    transaction: async (_, { transactionId }, context) => {
      const { transaction } = await canAccessTransaction(transactionId, context);
      return transaction;
    },
    ...
    patientTransactions: async (_, { userId, limit = 20, offset = 0 }, context) => {
    ...
    transactionsByType: async (_, { type, limit = 20, offset = 0 }, context) => {
      await isAdmin(context);
      return await TransactionModel.getByType(type.toLowerCase(), limit, offset);
    },

    // Get transactions by status (admin only)
    transactionsByStatus: async (_, { status, limit = 20, offset = 0 }, context) => {
      await isAdmin(context);
      return await TransactionModel.getByStatus(status.toLowerCase(), limit, offset);
    },
    ...
    searchTransactions: async (_, { filter, limit = 20, offset = 0 }, context) => {
      const user = isAuthenticated(context);
      const userDoc = await UserModel.getById(user.uid);
      ...
      return await TransactionModel.searchTransactions(searchFilter, limit, offset);
    },
    ...
    allTransactionStats: async (_, __, context) => {
      await isAdmin(context);
      return await TransactionModel.getTransactionStats();
    },

    // Get all refund statistics (admin only)
    allRefundStats: async (_, __, context) => {
      await isAdmin(context);
      return await RefundModel.getRefundStats();
    },
  },
  ...
  const user = await UserModel.getById(context.user.uid);       
    ...
    updateTransactionStatus: async (_, { transactionId, status, additionalData }, context) => {
      await isAdmin(context); // Only admin can update status directly
      
      const parsedData = additionalData ? JSON.parse(additionalData) : {};
      return await TransactionModel.updateStatus(transactionId, status.toLowerCase(), parsedData);
    },
    ...
    requestRefund: async (_, { input }, context) => {
      ...
      const originalTransaction = await TransactionModel.getById(input.originalWalletTransactionId);
    ...
    processRefund: async (_, { refundId, transactionRef }, context) => {
      ...
      return await RefundModel.updateStatus(refundId, 'processed', { transactionRef });
    },
    ...
    processDeposit: async (_, { transactionId, chapaRef, amount }, context) => {
      ...
      // Update patient wallet balance
      const patientProfile = await PatientProfileModel.getById(transaction.userId);
    ...
    processAppointmentPayment: async (_, { appointmentId, amount }, context) => {
      ...
      const patientProfile = await PatientProfileModel.getById(appointment.userId);
    ...
    cancelTransaction: async (_, { transactionId }, context) => {
      const { transaction, user } = await canAccessTransaction(transactionId, context);
