...
const appointmentResolvers = {
  ...
  Query: {
    ...
    appointment: async (_, { appointmentId }, context) => {
      const { appointment } = await canAccessAppointment(appointmentId, context);
      return appointment;
    },
    ...
    patientAppointments: async (_, { patientId, limit = 20, offset = 0 }, context) => {
      const user = isAuthenticated(context);
      const userDoc = await UserModel.getById(user.uid);
      
      // Admin can view any patient's appointments, patients can only view their own
      if (userDoc.role === "admin" || (userDoc.role === "patient" && user.uid === patientId)) {
        return await AppointmentModel.getByPatientId(patientId || user.uid, limit, offset);
      }
    ...
    doctorAppointments: async (_, { doctorId, limit = 20, offset = 0 }, context) => {
      const user = isAuthenticated(context);
      const userDoc = await UserModel.getById(user.uid);
      
      // Admin can view any doctor's appointments, doctors can only view their own
      if (userDoc.role === "admin" || (userDoc.role === "doctor" && user.uid === doctorId)) {
        return await AppointmentModel.getByDoctorId(doctorId || user.uid, limit, offset);
      }
    ...
    appointmentsByStatus: async (_, { status, limit = 20, offset = 0 }, context) => {
      await isAdmin(context); // Only admins can query by status across all appointments
      return await AppointmentModel.getByStatus(status.toLowerCase(), limit, offset);
    },
    ...
    upcomingAppointments: async (_, { limit = 10 }, context) => {
      ...
      if (userDoc.role === "patient" || userDoc.role === "doctor") {
        return await AppointmentModel.getUpcoming(user.uid, userDoc.role, limit);
      }
    ...
    searchAppointments: async (_, { filter, limit = 20, offset = 0 }, context) => {
      const user = isAuthenticated(context);
      const userDoc = await UserModel.getById(user.uid);
      ...
      return await AppointmentModel.searchAppointments(searchFilter, limit, offset);
    },
    ...
    appointmentStats: async (_, __, context) => {
      ...
      if (userDoc.role === "patient" || userDoc.role === "doctor") {
        return await AppointmentModel.getAppointmentStats(user.uid, userDoc.role);
      }
  ...
  Mutation: {
    ...
    createAppointment: async (_, { input }, context) => {
      ...
      const slot = await DoctorAvailabilitySlotModel.getById(input.associatedSlotId);
      ...
      // Check if patient has sufficient balance (this will be handled in frontend as mentioned)
      const patientProfile = await PatientProfileModel.getById(user.uid);
      if (patientProfile.telehealthWalletBalance < doctorProfile.pricePerSession) {
      ...
      // Mark slot as booked
      await DoctorAvailabilitySlotModel.updateSlot(input.associatedSlotId, {
        isBooked: true,
    ...
    updateAppointment: async (_, { input }, context) => {
      const { appointment, user } = await canAccessAppointment(input.appointmentId, context);
      
      // Only patients can update appointment details
      if (user.role !== "patient") {
        throw new ForbiddenError("Only patients can update appointment details");
      }
      
      // Can only update if appointment is still in requested status
      if (appointment.status !== "requested") {
        throw new UserInputError("Cannot update appointment after it has been processed");
      }
    ...
    updateAppointmentStatus: async (_, { input }, context) => {
      ...
      if (!allowedStatuses.includes(newStatus)) {
        throw new ForbiddenError(`Cannot change status from ${appointment.status} to ${newStatus}`);
      }
    ...
    cancelAppointment: async (_, { appointmentId, reason }, context) => {
      const { appointment, user } = await canAccessAppointment(appointmentId, context);
      ...
      const cancelStatus = user.role === 'doctor' ? 'cancelled_doctor' : 'cancelled_patient';
      
      // Free up the associated slot
      if (appointment.associatedSlotId) {
        await DoctorAvailabilitySlotModel.updateSlot(appointment.associatedSlotId, {
          isBooked: false,
          appointmentId: null,
          patientId: null
        });
    ...
    confirmAppointment: async (_, { appointmentId }, context) => {
      ...
      return await AppointmentModel.updateStatus(appointmentId, 'confirmed', 'doctor');
    },
    ...
    startAppointment: async (_, { appointmentId }, context) => {
      ...
      return await AppointmentModel.updateStatus(appointmentId, 'in_progress', 'doctor');
    },
    ...
    completeAppointment: async (_, { appointmentId, notes }, context) => {
      ...
      if (appointment.status !== 'in_progress') {
        throw new UserInputError("Can only complete appointments that are in progress");
      }
    ...
    markNoShow: async (_, { appointmentId, reason }, context) => {
      ...
      if (!['upcoming', 'confirmed'].includes(appointment.status)) {
        throw new UserInputError("Can only mark upcoming or confirmed appointments as no-show");
      }
    ...
    rescheduleAppointment: async (_, { appointmentId, newStartTime, newEndTime, newSlotId }, context) => {
      const user = await isPatient(context);
      const { appointment } = await canAccessAppointment(appointmentId, context);
      ...
      if (!reschedulableStatuses.includes(appointment.status)) {
        throw new UserInputError("Cannot reschedule appointment in current status");
      }
